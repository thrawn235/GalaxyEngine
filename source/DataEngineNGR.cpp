...
{
	this->engine->debug->PrintString( "DataEngine:Get Data...\n" );

	this->engine->debug->PrintString( "loading id: %i...\n", id );
	//data already loaded ?
	this->engine->debug->PrintString( "dataBlocks.size: %i...\n", dataBlocks.size() );
	for( unsigned int i = 0; i < dataBlocks.size(); i++ )
	{
		if( dataBlocks[i].id == id )
		{
			//yes
			this->engine->debug->PrintString( "ID found asset already loaded...\n" );
